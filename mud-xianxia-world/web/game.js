const SAVE_KEY = "mud_xianxia_world_v2";
...
function saveWorld() { state.lastSavedAt = Date.now(); localStorage.setItem(SAVE_KEY, JSON.stringify(state)); }
...
function advanceOneDay() {
  state.day++;
  if (state.day > 90) { state.day = 1; state.seasonIndex++; if (state.seasonIndex > 3) { state.seasonIndex = 0; state.year++; ageActors(); } }
  executeOrders(); npcAutonomy(); sectDynamics(); naturalRecovery(); checkDeaths();
}
...
function chooseNpcAction(npc) {
  ...
  if (npc.temperament === "好战" && roll(22)) return "ambush";
  if (npc.temperament === "阴狠" && roll(18)) return "ambush";
  if (npc.temperament === "贪婪" && roll(28)) return "askItem";
  if (npc.temperament === "温和" && roll(30)) return "socialize";
  if (npc.temperament === "商人" && roll(45)) return "work";
  if (npc.temperament === "好奇" && roll(30)) return "explore";
...
function chooseNpcTarget(npc, action) {
  ...
  if (action === "ambush") {
    const samePlace = others.filter(a => a.location === npc.location);
    const pool = samePlace.length ? samePlace : others;
    return [...pool].sort((a, b) => scoreVictim(npc, b) - scoreVictim(npc, a))[0];
  }
  if (["socialize", "askItem", "inviteTeam"].includes(action)) {
    const samePlace = others.filter(a => a.location === npc.location);
    const pool = samePlace.length ? samePlace : others;
    return [...pool].sort((a, b) => getRelation(state, npc.id, b.id) - getRelation(state, npc.id, a.id))[0];
  }
  return pick(others);
}
...
function runAction(actor, action, target, fromOrder) {
  ...
  if (["socialize", "askItem", "inviteTeam", "ambush"].includes(action) && target && target.alive && actor.location !== target.location) {
    moveTo(actor, target.location, `${actor.name} 为了${ACTION_LABELS[action]}前往 ${getLocationName(target.location)}。`);
    if (fromOrder) addLog(state, `${actor.name} 与 ${target.name} 不在同一地点，先移动到 ${getLocationName(target.location)}。`, "move");
    return;
  }
  if (action === "cultivate") return cultivate(actor, fromOrder);
  if (action === "meditate") return meditate(actor, fromOrder);
  if (action === "work") return work(actor, fromOrder);
  if (action === "explore") return explore(actor, fromOrder);
  if (action === "socialize") return socialize(actor, target, fromOrder);
  if (action === "askItem") return askItem(actor, target, fromOrder);
  if (action === "inviteTeam") return inviteTeam(actor, target, fromOrder);
  if (action === "ambush") return ambush(actor, target, fromOrder);
}
...
function moveForAction(actor, action) {
  if (action === "work") return moveTo(actor, actor.wealth < 80 ? "market" : pick(["market", "capital", "east_market"]));
  if (action === "cultivate") return moveTo(actor, SECTS[actor.sect]?.home || "starter_village");
  if (action === "explore") return moveTo(actor, pick(["black_forest", "spirit_mine", "icefield", "sea_islands", "southern_mountains"]));
}

function cultivate(actor, fromOrder) {
  moveForAction(actor, "cultivate");
  const loc = getLocation(actor.location); const gain = randInt(5, 12) + actor.realm + Math.floor((loc?.qi || 4) / 2);
  actor.exp += gain; actor.qi = clamp(actor.qi - randInt(3, 8), 0, actor.maxQi);
  if (actor.qi <= 5 && roll(18)) suffer(actor, randInt(3, 8), "强行修炼导致气血逆行");
  const need = 80 + actor.realm * 45;
  if (actor.exp >= need && actor.realm < REALM_NAMES.length - 1) { actor.exp -= need; actor.realm++; actor.maxHp += 12; actor.maxQi += 10; actor.hp = actor.maxHp; actor.qi = actor.maxQi; actor.lifespan += randInt(4, 10); actor.reputation += 2; addLog(state, `${actor.name} 在 ${getLocationName(actor.location)} 突破到 ${REALM_NAMES[actor.realm]}。`, "breakthrough"); }
  else if (fromOrder && roll(18)) addLog(state, `${actor.name} 在 ${getLocationName(actor.location)} 修炼，修为增加 ${gain}。`, "action");
}
...
function renderWorld() { const alive = state.actors.filter(a => a.alive).length; const dead = state.actors.length - alive; const sectSummary = Object.keys(SECTS).filter(s => s !== "无门派").map(s => `${s}${getSectMembers(s).length}`).join("｜"); el.worldDate.textContent = `第 ${state.year} 年 ${getSeason(state)} 第 ${state.day} 日`; el.worldSub.textContent = `存活 ${alive} 人，死亡 ${dead} 人，离线命令 ${state.orders.length} 条｜门派：${sectSummary}`; }
...
function renderOrders() { if (!state.orders.length) { el.ordersList.innerHTML = `<div class="empty">暂无离线命令。</div>`; return; } el.ordersList.innerHTML = state.orders.map(o => { const a = findActor(o.actorId); const t = o.targetId ? findActor(o.targetId) : null; return `<div class="order"><div class="order-title"><strong>${a ? escapeHtml(a.name) : "未知"}：${ACTION_LABELS[o.action]}</strong><span class="badge">剩余 ${o.remainingDays}/${o.totalDays} 天</span></div><div class="order-meta">${t ? `目标：${escapeHtml(t.name)}｜位置：${getLocationName(t.location)}` : "无指定目标"}</div></div>`; }).join(""); }
