...
离线任务与进度
行动队列、重复执行、暂停和失败重试
命令面板、模糊搜索、收藏、最近记录和键盘快捷操作
世界模板选择、备份、安全重置和玩家重建
世界人口、排行榜、活动与运行诊断洞察
工作区面板搜索、跳转、固定、折叠和紧凑模式
保存和读取世界
存档列表、名称、备注和读取确认
持续运行自动存档状态
自动刷新
...
命令面板支持中英文模糊搜索、方向键选择、Enter 执行、收藏和最近命令排序。收藏、最近记录及快捷键开关保存在 `localStorage`。

默认单键快捷操作仅在焦点不位于输入框、文本框、下拉框或可编辑元素时生效：
...
命令还可以导航至任务、商店、角色、存档、模板、世界洞察、持续运行、工作区导航和 GM 面板，并调用保存、快照导出、行动队列及持续运行控制。

## 世界模板管理

本地 API Server 通过模板包装器启动，并增加：
...
配置新世界 ID 和初始化 tick
重置前创建备份
保留账号、Session 和 API 审计
持续运行中自动暂停
重置后在模板默认地点重建当前玩家和角色
显示备份文件、模板和新世界结果
```

开启鉴权后只有 `gm/admin` 可以读取和重置模板。运行循环处于 busy 状态时重置返回 `409`；循环正在运行且未明确允许暂停时也返回 `409`。
...
```text
世界 tick、存活人口、平均力量和幸福度
玩家、任务、命令、物品和商店数量
按地点的人口分布
实体、城市、组织和文明排行
日志、遭遇、命令和世界报告活动流
运行上限、治理、冲突、基础设施和进程诊断
```

支持手动刷新、10/30/60 秒自动刷新、活动搜索、来源筛选、复制摘要和下载快照 JSON。
