...
async function handleApiRequest(req, res, api, options) {
  ...
  if (method === 'GET' && pathname === '/admin/runtime') { requireWorldControlIfNeeded(options, auth); return writeJson(res, 200, ok(getRuntimeSummary(createWorldRuntime(api.getWorld(), { maxTicks: 0 })))); }
