...
function createWorldApiServer(worldInput = null, options = {}) {
  const opts = mergeOptions(DEFAULT_API_OPTIONS, options || {});
  ...
  if (loopOptions.autosaveEveryTicks && !loopOptions.autosavePath) {
    loopOptions.autosavePath = opts.defaultSavePath;
  }
...
async function handleApiRequest(req, res, api, options) {
  ...
  if (method === 'GET' && (
    ...
    return serveClientFile(res, pathname, options);
  }

  if (method === 'GET' && pathname === '/health') {
    return writeJson(res, 200, health(api.getWorld(), api));
  }
  if (method === 'GET' && pathname === '/world') {
    return writeJson(res, 200, ok(queryWorld(api.getWorld(), { type: 'world' })));
  }
  if (method === 'GET' && pathname === '/snapshot') {
    return writeJson(res, 200, ok(createWorldSnapshot(api.getWorld())));
  }
  if (method === 'GET' && pathname === '/saves') {
    requireWorldControlIfNeeded(options, auth);
    return writeJson(res, 200, ok({
      saves: listSaves(parsed.searchParams.get('dir') || undefined),
    }));
  }
  if (method === 'GET' && pathname === '/stream') {
    return openTickStream(req, res, api);
  }

  if (method === 'GET' && pathname === '/admin/status') {
    requireWorldControlIfNeeded(options, auth);
    return writeJson(res, 200, ok(adminStatus(api)));
  }
  ...
  if (method === 'GET' && pathname === '/admin/loop') {
    requireWorldControlIfNeeded(options, auth);
    return writeJson(res, 200, ok(getRuntimeLoopSummary(api.runtimeLoop)));
  }
  if (method === 'POST' && pathname === '/admin/loop/start') {
    requireWorldControlIfNeeded(options, auth);
    const body = await readJsonBody(req, options);
    if (Object.keys(body || {}).length) {
      configureRuntimeLoop(api.runtimeLoop, body.options || body, { reschedule: false });
    }
    const summary = startRuntimeLoop(api.runtimeLoop);
    api.broadcast({
      type: 'runtime.loop.state',
      action: 'start',
      worldId: api.getWorld().id,
      tick: api.getWorld().tick,
      summary,
  ...
  if (method === 'POST' && pathname === '/admin/loop/step') {
    requireWorldControlIfNeeded(options, auth);
    const body = await readJsonBody(req, options);
    const report = stepRuntimeLoop(
      api.runtimeLoop,
      Math.max(1, Number(body.ticks || 1)),
      { source: 'api_step' },
    );
    if (!report.ok) {
      return writeJson(res, 409, {
        ok: false,
        error: report.reason || report.error?.message || 'runtime_loop_step_failed',
        data: report,
      });
    }
    return writeJson(res, 200, ok({
      report,
      summary: getRuntimeLoopSummary(api.runtimeLoop),
    }));
  }
  if (method === 'GET' && pathname === '/admin/connections') {
    requireWorldControlIfNeeded(options, auth);
    return writeJson(res, 200, ok({
      streams: api.streams.size,
      sockets: api.sockets.size,
    }));
  }
  if (method === 'GET' && pathname === '/admin/audit') {
    requireWorldControlIfNeeded(options, auth);
    return writeJson(res, 200, ok({
      stats: getApiAuditStats(api.getWorld()),
      log: getApiAuditLog(api.getWorld(), {
        limit: Number(parsed.searchParams.get('limit') || 100),
      }),
    }));
  }
  if (method === 'GET' && pathname === '/admin/errors') {
    requireWorldControlIfNeeded(options, auth);
    return writeJson(res, 200, ok({
      errors: getApiErrors(api.getWorld(), {
  ...
  if (method === 'POST' && pathname === '/accounts') {
    ...
    return writeJson(res, 201, ok(account));
  }
  ...
  if (method === 'GET' && accountMatch) {
    ...
    return writeJson(res, 200, ok(getAccountView(api.getWorld(), accountId)));
  }
  ...
  if (method === 'GET' && pathname === '/session') {
    return writeJson(
      ...
      auth
        ? ok({
          session: sanitizeSessionForApi(auth.session),
          account: getAccountView(api.getWorld(), auth.account.id),
        })
        : { ok: false, error: 'invalid_session' },
    );
  }
  if (method === 'POST' && pathname === '/sessions/revoke') {
    const body = await readJsonBody(req, options);
    const token = body.token || bearerToken(req);
    const session = revokeSession(api.getWorld(), token, body.reason || 'api_revoke');
    return writeJson(
      res,
      session ? 200 : 404,
      session
        ? ok({ session: sanitizeSessionForApi(session) })
  ...
  if (method === 'GET' && playerMatch) {
    ...
    return writeJson(res, 200, ok(queryWorld(api.getWorld(), {
      type: 'player',
      playerId,
    })));
  }
  ...
  if (method === 'GET' && dashboardMatch) {
    ...
    req.apiPlayerId = playerId;
    requirePlayerAccessIfNeeded(options, auth, playerId);
    return writeJson(res, 200, ok(getPlayerDashboard(api.getWorld(), playerId, {
      limit: Number(parsed.searchParams.get('limit') || 20),
    })));
  }

  const actionsMatch = pathname.match(/^\/players\/([^/]+)\/actions$/);
  if (method === 'POST' && actionsMatch) {
    const playerId = decodeURIComponent(actionsMatch[1]);
    req.apiPlayerId = playerId;
    requirePlayerAccessIfNeeded(options, auth, playerId);
    const body = await readJsonBody(req, options);
  ...
  const playerDetailMatch = pathname.match(
    /^\/players\/([^/]+)\/(inventory|quests|journal|map|shop|board|encounters|offline)$/,
  );
  if (method === 'GET' && playerDetailMatch) {
    ...
    req.apiPlayerId = playerId;
    requirePlayerAccessIfNeeded(options, auth, playerId);
    return writeJson(res, 200, ok(queryPlayerDetail(
      api.getWorld(),
      playerId,
      detail,
      parsed,
  ...
  if (method === 'GET' && offlineMatch) {
    ...
    return writeJson(res, 200, ok({
      playerId,
      offlineCommands: getPlayerOfflineCommands(api.getWorld(), playerId, { limit: 50 }),
    }));
  }

  if (method === 'POST' && pathname === '/players') {
    const body = await readJsonBody(req, options);
    if (body.accountId) requireAccountAccessIfNeeded(options, auth, body.accountId);
    const result = createPlayerWithCharacter(api.getWorld(), body || {});
    if (body.accountId) linkPlayerToAccount(api.getWorld(), body.accountId, result.player.id);
    req.apiPlayerId = result.player.id;
    api.broadcast({
      type: 'player.created',
      worldId: api.getWorld().id,
  ...
  if (method === 'POST' && pathname === '/offline') {
    ...
    req.apiPlayerId = playerId;
    requirePlayerAccessIfNeeded(options, auth, playerId);
    const command = scheduleOfflineCommand(
      api.getWorld(),
      playerId,
      body.command || body,
      body.options || {},
  ...
  if (method === 'POST' && pathname === '/tick') {
    ...
    const runtime = createWorldRuntime(api.getWorld(), {
      maxTicks: ticks,
      tickBatch: 1,
      ...(body.runtime || {}),
    });
...
function queryPlayerDetail(world, playerId, detail, parsed) {
  if (detail === 'offline') {
    return queryWorld(world, {
      type: 'offline',
      playerId,
      options: { limit: Number(parsed.searchParams.get('limit') || 50) },
    });
  }
  if (detail === 'journal') {
    return queryWorld(world, {
...
function adminStatus(api) {
  const world = api.getWorld();
  return {
    health: health(world, api),
    audit: getApiAuditStats(world),
    accounts: getAccountStats(world),
    runtime: getRuntimeSummary(createWorldRuntime(world, { maxTicks: 0 })),
    loop: getRuntimeLoopSummary(api.runtimeLoop),
    limits: queryWorld(world, { type: 'world' }).limits,
  };
}

function getRequestAuth(world, req, parsed = null) {
  const token = bearerToken(req) || parsed?.searchParams?.get('token') || null;
  return token ? validateSession(world, token) : null;
}

function requireAccountAccessIfNeeded(options, auth, accountId) {
  if (!options.requireAuth) return;
  const sessionAuth = requireSession(auth);
  requirePermission(canAccessAccount(sessionAuth.account, accountId), 'account_forbidden');
}

function requirePlayerAccessIfNeeded(options, auth, playerId) {
  if (!options.requireAuth) return;
  const sessionAuth = requireSession(auth);
  requirePermission(canAccessPlayer(sessionAuth.account, playerId), 'player_forbidden');
}
...
async function readJsonBody(req, options = {}) {
  ...
  for await (const chunk of req) {
    size += chunk.length;
    if (size > (options.maxBodyBytes || DEFAULT_API_OPTIONS.maxBodyBytes)) {
      throw apiError(413, 'request_body_too_large');
    }
    chunks.push(chunk);
  }
...
function requiredBody(body, key) {
  if (!body || body[key] === undefined || body[key] === null || body[key] === '') {
    throw apiError(400, `Request body requires ${key}`);
  }
  return body[key];
}
...
function health(world, api = null) {
  return {
    ...
    streams: api?.streams?.size || 0,
    sockets: api?.sockets?.size || 0,
    runtimeLoop: api?.runtimeLoop ? getRuntimeLoopSummary(api.runtimeLoop) : null,
  };
}

function summarizeLoopCycle(report) {
  if (!report) return null;
  return {
    ok: Boolean(report.ok),
    cycle: report.cycle || null,
    tickBefore: report.tickBefore ?? null,
    tickAfter: report.tickAfter ?? null,
    ticks: report.ticks ?? 0,
    durationMs: report.durationMs ?? 0,
    autosave: report.autosave
      ? { file: report.autosave.file, tick: report.autosave.tick }
      : null,
