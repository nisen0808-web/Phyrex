...
const DEFAULT_API_OPTIONS = { port: 8790, host: '127.0.0.1', seedTicks: 10, maxBodyBytes: 1024 * 1024, defaultSavePath: 'world-engine/output/api-world-save.json' };
...
async function handleApiRequest(req, res, api, options) {
  ...
  if (method === 'POST' && pathname === '/accounts') {
    ...
    api.broadcast({ type: 'account.created', worldId: api.getWorld().id, tick: api.getWorld().tick, accountId: account.id });
    return writeJson(res, 201, ok(account));
  }

  const accountMatch = pathname.match(/^\/accounts\/([^/]+)$/);
  if (method === 'GET' && accountMatch) return writeJson(res, 200, ok(getAccountView(api.getWorld(), decodeURIComponent(accountMatch[1]))));
  ...
  if (method === 'POST' && pathname === '/sessions') {
    ...
    api.broadcast({ type: 'session.created', worldId: api.getWorld().id, tick: api.getWorld().tick, accountId, sessionId: session.id });
    return writeJson(res, 201, ok({ token: session.token, session: sanitizeSessionForApi(session), account: getAccountView(api.getWorld(), accountId) }));
  }

  if (method === 'GET' && pathname === '/session') {
    const token = bearerToken(req) || parsed.searchParams.get('token');
    const validated = token ? validateSession(api.getWorld(), token) : null;
    return writeJson(res, validated ? 200 : 401, validated ? ok({ session: sanitizeSessionForApi(validated.session), account: getAccountView(api.getWorld(), validated.account.id) }) : { ok: false, error: 'invalid_session' });
  }
  ...
  if (method === 'POST' && pathname === '/sessions/revoke') {
    ...
    return writeJson(res, session ? 200 : 404, session ? ok({ session: sanitizeSessionForApi(session) }) : { ok: false, error: 'missing_session' });
  }
  ...
  if (method === 'GET' && offlineMatch) { const playerId = decodeURIComponent(offlineMatch[1]); return writeJson(res, 200, ok({ playerId, offlineCommands: getPlayerOfflineCommands(api.getWorld(), playerId, { limit: 50 }) })); }
  ...
  if (method === 'POST' && pathname === '/commands') { const body = await readJsonBody(req, options); const playerId = requiredBody(body, 'playerId'); const command = body.command || body; const result = executePlayerCommand(api.getWorld(), playerId, command, body.options || {}); api.broadcast({ type: 'command', worldId: api.getWorld().id, tick: api.getWorld().tick, playerId, commandId: result.command.id, status: result.command.status }); return writeJson(res, result.result.ok ? 200 : 400, ok(result)); }
  if (method === 'POST' && pathname === '/offline') { const body = await readJsonBody(req, options); const playerId = requiredBody(body, 'playerId'); const command = scheduleOfflineCommand(api.getWorld(), playerId, body.command || body, body.options || {}); api.broadcast({ type: 'offline.queued', worldId: api.getWorld().id, tick: api.getWorld().tick, playerId, offlineCommandId: command.id }); return writeJson(res, 201, ok(command)); }
  if (method === 'POST' && pathname === '/tick') { const body = await readJsonBody(req, options); const ticks = Math.max(1, Number(body.ticks || 1)); const runtime = createWorldRuntime(api.getWorld(), { maxTicks: ticks, tickBatch: 1, ...(body.runtime || {}) }); const summary = runWorldRuntime(runtime, body.options || {}); api.broadcast({ type: 'tick', worldId: api.getWorld().id, tick: api.getWorld().tick, summary }); return writeJson(res, 200, ok(summary)); }
  if (method === 'POST' && pathname === '/runtime/run') { const body = await readJsonBody(req, options); const runtime = createWorldRuntime(api.getWorld(), { ...(body.runtime || body || {}) }); const summary = runWorldRuntime(runtime, body.options || {}); api.broadcast({ type: 'runtime', worldId: api.getWorld().id, tick: api.getWorld().tick, summary }); return writeJson(res, 200, ok(summary)); }
  if (method === 'POST' && pathname === '/save') { const body = await readJsonBody(req, options); const filePath = body.filePath || body.path || options.defaultSavePath; const save = saveWorld(api.getWorld(), filePath, body.options || {}); api.broadcast({ type: 'save', worldId: api.getWorld().id, tick: api.getWorld().tick, file: save.file }); return writeJson(res, 200, ok(save)); }
  if (method === 'POST' && pathname === '/load') { const body = await readJsonBody(req, options); const filePath = body.filePath || body.path || options.defaultSavePath; const loaded = loadWorld(filePath, body.options || {}); api.setWorld(loaded.world); api.broadcast({ type: 'load', worldId: loaded.worldId, tick: loaded.tick }); return writeJson(res, 200, ok({ file: loaded.file, worldId: loaded.worldId, tick: loaded.tick, savedAt: loaded.savedAt })); }
