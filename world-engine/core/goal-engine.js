...
function createGoal(input = {}) {
  ...
  return {
    id: input.id || `goal_${Date.now()}_${Math.random().toString(16).slice(2)}`,
    type: input.type,
...
function assignGoal(world, entityId, goalInput) {
  ...
  const goal = createGoal({ ...goalInput, createdAt: world.tick, updatedAt: world.tick });
...
function evaluateGoals(world, entityId) {
  ...
  for (const goal of entity.goals) {
    ...
    const completed = template && typeof template.completion === 'function'
      ? template.completion(entity, goal, world)
      : goal.progress >= 100;
...
function calculateGoalProgress(world, entity, goal) {
  ...
  if (goal.type === 'gain_resources') {
    const resource = goal.payload.resource || 'currency';
    const amount = Number(goal.payload.amount || 100);
    return clamp((Number(entity.resources[resource] || 0) / amount) * 100, 0, 100);
  }
  if (goal.type === 'gain_power') {
    const target = Number(goal.payload.power || 100);
    return clamp((Number(entity.stats.power || 0) / target) * 100, 0, 100);
  }
...
function spawnFollowUpGoals(world, entity, completedGoal) {
  if (completedGoal.type === 'gain_resources' && completedGoal.scope !== GOAL_SCOPE.DREAM) {
    assignGoal(world, entity.id, {
      ...
      payload: {
        resource: completedGoal.payload.resource || 'currency',
        amount: Number(completedGoal.payload.amount || 100) * 2,
      },
      tags: ['follow_up'],
    });
  }
  ...
  if (completedGoal.type === 'gain_power') {
    assignGoal(world, entity.id, {
      ...
      payload: { power: Number(completedGoal.payload.power || 100) * 2 },
      tags: ['escalated_dream'],
    });
  }
}
...
function chooseActiveGoal(world, entityId) {
  ...
  return entity.goals
    ...
    .sort((a, b) => scoreGoal(entity, b) - scoreGoal(entity, a))[0] || null;
}
...
function scoreGoal(entity, goal) {
  ...
  return goal.priority * scopeWeight + urgency * 0.2 + dreamBoost;
}
...
function goalToActionIntent(world, entityId, goal) {
  ...
  if (goal.type === 'recover') {
    return { type: 'rest', actorId: entity.id, duration: 1, priority: goal.priority, payload: { health: 12, energy: 20, goalId: goal.id } };
  }

  if (goal.type === 'survive') {
    if (entity.stats.health < entity.stats.maxHealth * 0.5) {
      return { type: 'rest', actorId: entity.id, duration: 1, priority: goal.priority, payload: { health: 10, energy: 10, goalId: goal.id } };
    }
    return { type: 'gather', actorId: entity.id, duration: 1, priority: goal.priority, payload: { resource: 'food', amount: 1, goalId: goal.id } };
  }

  if (goal.type === 'gain_resources') {
    return { type: 'work', actorId: entity.id, duration: 1, priority: goal.priority, payload: { resource: goal.payload.resource || 'currency', amount: 5, energyCost: 5, goalId: goal.id } };
  }

  if (goal.type === 'build_relationship' && goal.targetId) {
    return { type: 'interact', actorId: entity.id, targetId: goal.targetId, duration: 1, priority: goal.priority, payload: { effect: 'social', amount: 3, goalId: goal.id } };
  }

  if (goal.type === 'gain_power') {
    return { type: 'work', actorId: entity.id, duration: 1, priority: goal.priority, payload: { resource: 'training', amount: 1, energyCost: 8, goalId: goal.id } };
  }

  if (goal.type === 'lead_faction') {
    return { type: 'interact', actorId: entity.id, duration: 1, priority: goal.priority, payload: { effect: 'social', amount: 2, goalId: goal.id } };
  }
