...
function main() {
  const world = createWorld({ id: 'stability-100-test-world' });
  ...
  assert.strictEqual(world.tick, 100, 'world should advance exactly 100 ticks');
  ...
  assert.ok(summary.counters.ticks >= 100, 'simulation counters should count ticks');
  ...
  console.log('100 tick stability test passed');
}
