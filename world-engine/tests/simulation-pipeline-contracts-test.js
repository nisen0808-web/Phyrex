...
function main() {
  ...
  assert.strictEqual(Object.keys(contracts).length, 31, 'every built-in, natural, ecology and consistency system should receive a contract');
  assert.ok(contracts['natural.world']);
  assert.ok(contracts['ecology.world']);
  assert.ok(contracts['world.consistency']);
  ...
  assert.ok(contracts['world.consistency'].postconditions.some(rule => rule.path === 'world.consistency.lastReport'));
  ...
  assert.strictEqual(coverage.systems, 31);
  assert.strictEqual(coverage.contracted, 31, 'kernel should attach contracts by default');
  ...
  assert.strictEqual(attachment.attached.length, 31);
  ...
  const consistencyEntry = report.kernel.order.indexOf('world.consistency');
  ...
  assert.ok(consistencyEntry > advanceEntry);
  assert.ok(finalizeEntry > consistencyEntry);
  ...
  assert.ok(completedContracts.includes('world.consistency'));
  assert.ok(completedContracts.includes('finalize.report'));

  const summary = getSystemContractSummary(world);
  assert.ok(summary.validations >= 14, 'completed systems should validate input, output and postconditions');
  ...
  assert.ok(summary.systems.some(system => system.id === 'world.consistency'));
...
function buildWorld(id = 'contract-pipeline-world') {
  ...
  registerLocation(world, { id: 'origin', name: 'Origin', resources: { food: 100, water: 100, wood: 100 } });
