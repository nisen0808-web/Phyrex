...
| 模块化模拟管线 | 96% -> 97% | 已新增 knowledge.culture_belief_flow 系统，可通过 runtime helper 接入 deterministic kernel。 |
...
| 重放与确定性验证 | 86% -> 88% | 文化信仰流已可通过 runtime helper 进入 schedule order，并输出 report.cultureBeliefFlow。 |
...
| 信息、记忆、文化、宗教 | 84% -> 88% | 文化与信仰扩散网络已进入 runtime helper，覆盖 schedule order、report.cultureBeliefFlow 和 counters。 |
...
culture-belief-flow-system-engine.js
culture-belief-flow-runtime-engine.js
...
simulation counters: cultureBeliefFlowLinks / cultureBeliefTransfers / beliefCultureInfluences / beliefOrganizationLinks
package.json 追加 culture-belief-flow runtime 回归测试
```
