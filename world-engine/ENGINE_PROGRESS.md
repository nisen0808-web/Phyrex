...
| 模块化模拟管线 | 93% | 治理响应已经能进入 civilization.processes，成为多 tick 过程。 |
| 系统 Contract 校验 | 83% | civilization.governance 和 civilization.processes 均覆盖关键输出结构。 |
| 源码确定性纯度审计 | 35% | 已有扫描器、baseline、allowlist 和测试；下一步接入核心目录 baseline。 |
| 状态哈希与差异定位 | 82% | 一致性层可在重放前后修复结构性坏状态，减少无意义漂移。 |
| 重放与确定性验证 | 80% | 自然、生态、一致性、治理环境响应和治理过程执行进入确定性管线路径。 |
...
| 治理、过程、冲突 | 56% -> 62% | 治理响应现在会进入过程系统，并在后续 tick 持续产生救灾、工程、配给、治安和动员效果。 |
| 治理环境响应层 | 100% | 已完成救灾、公共工程、配给、治安维护、税率调整和组织动员。 |
| 治理过程执行层 | 0% -> 100% | 已完成治理响应日志消费、governance_response 过程生成、持续效果、过程推进和确定性管线测试。 |
| 信息、记忆、文化、宗教 | 55% | 已接入管线；仍需传播网络和文化变迁细化。 |
| 文明、科技、基础设施 | 50% | 已接入管线；仍需时代、扩散和依赖网络。 |
| 自然、气候、生态 | 55% | 自然世界和生态主模块已完成基础版，并能向人口、城市、经济、AI 目标和治理系统提供压力数据。 |
...
governance-process-engine.js
governance-process-execution-test.js
GOVERNANCE_PROCESS_EXECUTION.md
world.processes.consumedGovernanceResponseIds
governance_response process type
disaster_relief / public_works / rationing / security_crackdown / tax_adjustment / mobilization 多 tick 过程效果
npm test 追加治理过程执行回归测试
```

## 下一批建议

```text
1. 让 conflict-engine 读取 security_crackdown 和 mobilization 过程，影响骚乱、镇压和组织冲突。
2. 让 opportunity-engine 读取治理环境摘要和治理过程，生成公共救灾、工程、贸易和迁徙机会。
3. 增加贸易流系统，根据城市风险和价格差自动移动资源。
4. 让 organization-engine 根据治理过程吸纳灾民、志愿者、工人和治安力量。
5. 基于 source-purity-engine 扫描核心目录并生成首个 baseline。
6. 增加系统级性能预算与采样。
```
