...
| 数据库配置与持久化 | 0% -> 35% | 已新增 database config、JSONL 本地数据库 store、世界存档读写、世界列表、事件记录和环境变量示例。 |
...
database-config-engine.js
database-engine.js
database-config-test.js
DATABASE_CONFIG.md
.env.database.example
WORLD_ENGINE_DB_PROVIDER / DIR / NAME / AUTO_CREATE
JSONL world save store
JSONL event store
package.json 追加 database-config 回归测试
```

## 下一批建议

```text
1. 将 API server 的 /save、/load、/saves 接入 database store。
