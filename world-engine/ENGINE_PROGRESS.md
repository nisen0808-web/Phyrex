...
| 模块化模拟管线 | 94% | 治理、过程、冲突、机会和经济贸易流系统已经形成连续联动。 |
| 系统 Contract 校验 | 84% | agency.opportunity 已覆盖治理机会生成路径，economy 继续保持 Contract 兼容。 |
| 源码确定性纯度审计 | 35% | 已有扫描器、baseline、allowlist 和测试；下一步接入核心目录 baseline。 |
| 状态哈希与差异定位 | 82% | 一致性层可在重放前后修复结构性坏状态，减少无意义漂移。 |
| 重放与确定性验证 | 83% | 治理环境、治理过程、冲突联动、机会生成和贸易流进入确定性管线路径。 |
...
| 经济系统 | 68% -> 76% | 经济系统已新增地点资源贸易流，会根据城市压力、地点资源和市场压力自动移动资源。 |
| 经济环境联动层 | 100% | 已完成产业风险、生产倍率、价格压力、产业状态、市场供需冲击和测试。 |
| 贸易流系统 | 0% -> 100% | 已完成贸易候选计算、资源移动、交易流水、贸易流摘要、统计和确定性管线测试。 |
...
```text
Trade flow system
trade-flow-engine.js
trade-flow-system-test.js
TRADE_FLOW_SYSTEM.md
world.economy.tradeFlows
world.economy.tradeFlowLog
world.economy.transactions[*].type = trade_flow
world.economy.stats.tradeFlowCount / tradeFlowVolume
food / water / wood / stone / metal / fuel / luxury 地点资源流动
extra-engine-tests.js 追加贸易流回归测试
```

## 下一批建议

```text
1. 让 organization-engine 根据治理过程吸纳灾民、志愿者、工人和治安力量。
2. 基于 source-purity-engine 扫描核心目录并生成首个 baseline。
3. 增加系统级性能预算与采样。
```
