...
| 模块化模拟管线 | 94% | 治理、过程、冲突和机会系统已经形成连续联动。 |
| 系统 Contract 校验 | 84% | agency.opportunity 已覆盖治理机会生成路径。 |
| 源码确定性纯度审计 | 35% | 已有扫描器、baseline、allowlist 和测试；下一步接入核心目录 baseline。 |
| 状态哈希与差异定位 | 82% | 一致性层可在重放前后修复结构性坏状态，减少无意义漂移。 |
| 重放与确定性验证 | 82% | 治理环境、治理过程、冲突联动和机会生成进入确定性管线路径。 |
...
| 治理、过程、冲突 | 68% -> 72% | 机会系统已读取治理环境、治理过程和冲突状态，生成公共救助、工程、补给、迁徙和协作机会。 |
...
| 机会治理联动层 | 0% -> 100% | 已完成治理环境、治理过程和冲突状态到公共机会的映射、去重和统计。 |
| 信息、记忆、文化、宗教 | 55% | 已接入管线；仍需传播网络和文化变迁细化。 |
| 文明、科技、基础设施 | 50% | 已接入管线；仍需时代、扩散和依赖网络。 |
| 自然、气候、生态 | 55% | 自然世界和生态主模块已完成基础版，并能向人口、城市、经济、AI 目标、治理、冲突和机会系统提供压力数据。 |
...
opportunity-governance-engine.js
opportunity-governance-linkage-test.js
OPPORTUNITY_GOVERNANCE_LINKAGE.md
governanceGenerated opportunity stats
payload.governanceOpportunityKey dedupe
public relief / relief supply route / migration support
relief work / public works contract / ration logistics / stabilization channel
conflict mediation
extra-engine-tests.js 聚合扩展测试
```

## 下一批建议

```text
1. 增加贸易流系统，根据城市风险和价格差自动移动资源。
2. 让 organization-engine 根据治理过程吸纳灾民、志愿者、工人和治安力量。
